...
int main(void)
{
	...
	*gpio_d_output &= ~(0xF << 12);
	...
	while (1) {
		...
		if(*gpio_a_input & 0x1) {
			speedMultiplierPreviousState = 0x1; // set button state to pressed
		} else {
			if(speedMultiplierPreviousState == 0x1) { // if button was released
				ledSpeedMultiplier += 2; //increase speed multiplier by 2
				if(ledSpeedMultiplier > 10) {
					ledSpeedMultiplier = 1; //reset after multiplier reaches 5
				}
			}
			speedMultiplierPreviousState = 0x0; // reset previous button state
		}
		...
		*gpio_d_output |= LEDS[ledIndex]; //set bit for current LED at ledIndex
		for(uint32_t ii = 0; ii < (ledSpeedMultiplier * ledSpeed); ii++) {} //artificial delay
